...
*** At the bottom of this script are commands to drop all thearch.loyalty_cohort tables.
...
drop table if exists arch.loyalty_cohort_filters
drop table if exists arch.loyalty_cohort_filter_sets
drop table if exists arch.loyalty_cohort_concept_type
drop table if exists arch.loyalty_cohort_patient_concept_type
drop table if exists arch.loyalty_cohort_patient_facts
drop table if exists arch.loyalty_cohort_patient_filter
drop table if exists arch.loyalty_cohort_patient_summary


create table arch.loyalty_cohort_filters (
	filter int primary key,
	filter_bit int,
	description varchar(100)
)

insert into arch.loyalty_cohort_filters (filter,description) values (1,'AgeSex')
insert into arch.loyalty_cohort_filters (filter,description) values (2,'Race')
insert into arch.loyalty_cohort_filters (filter,description) values (3,'SameState')
insert into arch.loyalty_cohort_filters (filter,description) values (4,'All6MonthPeriods')
insert into arch.loyalty_cohort_filters (filter,description) values (5,'All1YearPeriods')
insert into arch.loyalty_cohort_filters (filter,description) values (6,'FirstLastYear')
insert into arch.loyalty_cohort_filters (filter,description) values (7,'Diagnoses')
insert into arch.loyalty_cohort_filters (filter,description) values (8,'Medications')
insert into arch.loyalty_cohort_filters (filter,description) values (9,'LabTests')
insert into arch.loyalty_cohort_filters (filter,description) values (10,'VitalSigns')
insert into arch.loyalty_cohort_filters (filter,description) values (11,'RoutineVisit')
insert into arch.loyalty_cohort_filters (filter,description) values (12,'OutpatientVisit')
insert into arch.loyalty_cohort_filters (filter,description) values (13,'Alive')
insert into arch.loyalty_cohort_filters (filter,description) values (14,'NoSmallFactCount')
insert into arch.loyalty_cohort_filters (filter,description) values (15,'FirstLast18Months')
insert into arch.loyalty_cohort_filters (filter,description) values (16,'AgeCutoffs')

update arch.loyalty_cohort_filters set filter_bit = power(2,filter-1)
...
create table arch.loyalty_cohort_filter_sets (
...
;with n as (
	...
	from (select filter-1 n from arch.loyalty_cohort_filters where filter < 9) a
		cross join (select filter-1 n from arch.loyalty_cohort_filters where filter < 9) b
		cross join (select filter-1 n from arch.loyalty_cohort_filters where filter < 9) c
		cross join (select filter-1 n from arch.loyalty_cohort_filters where filter < 9) d
		cross join (select filter-1 n from arch.loyalty_cohort_filters where filter < 9) e
		cross join (select filter-1 n from arch.loyalty_cohort_filters where filter < 3) f
)
insert into arch.loyalty_cohort_filter_sets (filter_set,num_filters,bit_list,filter_list)
	select n.n, count(*) m, '', ''
	from n inner join arch.loyalty_cohort_filters f
...
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 1
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 2
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 3
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 4
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 5
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 6
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 7
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 8
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 9
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 10
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 11
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 12
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 13
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 14
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 15
update s set s.bit_list = s.bit_list + ','+cast(f.filter as varchar(10)), s.filter_list = s.filter_list + ',' + f.description
	from arch.loyalty_cohort_filter_sets s inner join arch.loyalty_cohort_filters f on s.filter_set & f.filter_bit = f.filter_bit and f.filter = 16

update arch.loyalty_cohort_filter_sets set bit_list = substring(bit_list,2,99999), filter_list = substring(filter_list,2,99999)

insert into arch.loyalty_cohort_filter_sets (filter_set,num_filters,bit_list,filter_list) values (0,0,'0','None')

--select *
--from arch.loyalty_cohort_filter_sets
--where filter_set in (61511,47559,47303)
...
create table arch.loyalty_cohort_concept_type (
...
insert into arch.loyalty_cohort_concept_type (concept_cd, concept_type)
	select concept_cd, concept_type
	from (
		select concept_cd,
			min(case when (concept_path like '\Diagnoses\%' or concept_path like '\ACT\DIAGNOSIS\%') and concept_cd in ('ICD9CM:V20.2','ICD9CM:V70.0','ICD9CM:V72.31','ICD10CM:Z00.129','ICD10CM:Z00.00','ICD10CM:Z01.411','ICD10CM:Z01.419') then 'C'
				when concept_path like '\Diagnoses\%' or concept_path like '\ACT\DIAGNOSIS\%' then 'D'
				when concept_path like '\ACT\MEDICATIONS\%' then 'M'
				when concept_path like '\ACT\LAB[S\]%' then 'L'
				when concept_path like '\ACT\PROCEDURES\%' then 'P'
				when concept_path like '\ACT\VITAL%' then 'V'
...
select concept_type, count(distinct concept_cd) from arch.loyalty_cohort_concept_type group by concept_type

-------------------------------------------------------------------------------------------
-- Determine how many facts each patient has per category and time period
-------------------------------------------------------------------------------------------

create table arch.loyalty_cohort_patient_concept_type (
...
starting on July 1, 2010, and going through June 30, 2014. Time period 0 spans 20170201
through 12/31/2010.
*/

/* ADJUST TO CURRENT TIME PERIOD FOR LOYALTY COHORT SCRIPT PROJECT
SELECT DATEADD(YY,-4,'20210201')
*/

insert into arch.loyalty_cohort_patient_concept_type (patient_num, time_period, concept_type, num_facts)
	select patient_num, time_period, concept_type, count(*) num_facts
	from (
		select f.patient_num,
			floor(datediff(mm,'20170201',start_date)/6) time_period,
			c.concept_type
		from observation_fact f
			inner join arch.loyalty_cohort_concept_type c
				on f.concept_cd = c.concept_cd
		where start_date >= '20170201' and start_date < '20210201'
	) t
...
select distinct inout_cd from VISIT_DIMENSION
...
alter table arch.loyalty_cohort_patient_concept_type add primary key (patient_num, time_period, concept_type)

-------------------------------------------------------------------------------------------
-- Determine the total number of facts for each patient
-------------------------------------------------------------------------------------------

create table arch.loyalty_cohort_patient_facts (
	patient_num int not null,
	num_facts int
)

insert into arch.loyalty_cohort_patient_facts
	select patient_num, sum(num_facts) num_facts
	from arch.loyalty_cohort_patient_concept_type
	where concept_type not in ('O','I','E')
	group by patient_num

alter table arch.loyalty_cohort_patient_facts add primary key (patient_num)
...
-- Create the table
create table arch.loyalty_cohort_patient_filter (
	patient_num int not null,
	filter int not null
)

-- All patients
insert into arch.loyalty_cohort_patient_filter (patient_num, filter)
select patient_num, 0
from patient_dimension
where SOURCESYSTEM_CD is null

-- AgeSex
insert into arch.loyalty_cohort_patient_filter (patient_num, filter)
	select patient_num, 1
	from patient_dimension
	where birth_date is not null and REPLACE(sex_cd,'DEM|SEX:','') in ('M','F')
...
insert into arch.loyalty_cohort_patient_filter (patient_num, filter)
	...
	from patient_dimension
	where zip_cd like '4[012]%' --zip_cd like '01%' or zip_cd like '02[0-7]%'
...
/* test/simplify: 
select distinct patient_num, 5 from (
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (0,1) intersect
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (2,3) intersect
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (4,5) intersect
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (6,7) 
)p
*/
...
/* test/simplify:
select distinct patient_num, 6 from (
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (0,1) intersect
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (6,7)
)p
...
insert into arch.loyalty_cohort_patient_filter (patient_num, filter)
	...
	from patient_dimension
	where vital_status_cd in ('ND') or death_date is null
...
insert into arch.loyalty_cohort_patient_filter (patient_num, filter)
	...
	from (
		select patient_num, a, s, f, ntile(10) over (partition by a, s order by f, patient_num) k
		from (
			select p.patient_num, floor(age_in_years_num/10) a, REPLACE(sex_cd,'DEM|SEX:','') s, isnull(f.num_facts,0) f
			from patient_dimension p
				inner join arch.loyalty_cohort_patient_facts f
					on p.patient_num = f.patient_num
		) t 
	) t
	where k > 1

select k, count(distinct patient_num) from (
  select patient_num, a, s, f, ntile(10) over (partition by a, s order by f, patient_num) k
		from (
			select p.patient_num, floor(age_in_years_num/10) a, REPLACE(sex_cd,'DEM|SEX:','') s, isnull(f.num_facts,0) f
			from patient_dimension p
...
select distinct patient_num, 15 
from (
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (0,1,2) intersect 
select patient_num from arch.loyalty_cohort_patient_concept_type where time_period in (5,6,7)
)p
*/
...
-- Add primary key
alter table arch.loyalty_cohort_patient_filter add primary key (patient_num, filter)
...
create table arch.loyalty_cohort_patient_summary (
	patient_num int not null,
	filter_set int,
	num_facts int,
	num_diagnoses int,
...
;with a as (
	...
	from arch.loyalty_cohort_patient_filter
	group by patient_num
), b as (
	select patient_num, 
...
insert into arch.loyalty_cohort_patient_summary
	select a.patient_num, a.Filters,
		...
		(case when REPLACE(sex_cd,'DEM|SEX:','') = 'F' then 'F'
			when REPLACE(sex_cd,'DEM|SEX:','') = 'M' then 'M'
			else NULL end)
	from a 
...
alter table arch.loyalty_cohort_patient_summary add primary key (patient_num)

create unique nonclustered index idx_fp on arch.loyalty_cohort_patient_summary(filter_set,patient_num)
...
select f.filter, f.description, isnull(a.n,0) patients
		from arch.loyalty_cohort_filters f
			left outer join a
				on f.filter = a.filter
	union all
	select 0, 'AllPatients', count(*)
		from arch.loyalty_cohort_patient_summary
	order by 1
...
select filter_set, 
		...
		sum(case when REPLACE(sex_cd,'DEM|SEX:','') = 'F' then 1 else 0 end) female_patients,
		sum(case when REPLACE(sex_cd,'DEM|SEX:','') = 'M' then 1 else 0 end) male_patients,
		isnull(sum(cast(age_group as bigint)),-1) sum_age_group
	from arch.loyalty_cohort_patient_summary
  WHERE filter_set in (61511,45127)
	group by filter_set 
  order by 2 desc

/* no clue what the sum of age group is supposed to accomplish in this final result 
   you're left with a big number, that's just the sum of each patient's age group [ floor(age/10) ] ?
   You have no idea how many of your patients are in the first age_group = 0, null is still present in the age_group data as well...

select age_Group,count(distinct patient_num) from arch.loyalty_cohort_patient_summary group by age_group
*/
  


-------------------------------------------------------------------------------------------
-- Number of patients who pass each filter set, broken down by age_group and REPLACE(sex_cd,'DEM|SEX:','')
-------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------
;with t as (
	select filter, description, filter_bit from arch.loyalty_cohort_filters
	union all
	select 17, 'None (All Patients)', 0
)
select t.filter, t.description, 
		count(*) num_patients, 
		count(*)/(select count(*)*1.0 total_patients from arch.loyalty_cohort_patient_summary) frac_patients,
	...
	from arch.loyalty_cohort_patient_summary s
...
;with t as (
	...
	from arch.loyalty_cohort_filters a
		cross join arch.loyalty_cohort_filters b
		inner join arch.loyalty_cohort_patient_summary t
...
select t.filter_set, count(*) num_patients, 
		...
		t.filter_list
	from arch.loyalty_cohort_patient_summary s
		inner join arch.loyalty_cohort_filter_sets t on s.filter_set & t.filter_set = t.filter_set
...
;with t as (
	...
	from @f g
		left outer join arch.loyalty_cohort_filters f
			on g.f = f.filter
)
...
/*
drop table arch.loyalty_cohort_filters
drop table arch.loyalty_cohort_filter_sets
drop table arch.loyalty_cohort_concept_type
drop table arch.loyalty_cohort_patient_concept_type
drop table arch.loyalty_cohort_patient_facts
drop table arch.loyalty_cohort_patient_filter
drop table arch.loyalty_cohort_patient_summary
*/