IF OBJECT_ID(N'DBO.usp_LoyaltyCohort_opt_10y') IS NOT NULL DROP PROCEDURE DBO.usp_LoyaltyCohort_opt_10y
GO

CREATE PROC DBO.usp_LoyaltyCohort_opt_10y
    @indexDate datetime
AS
...
IF OBJECT_ID(N'DBO.loyalty_dev_summary_10y', N'U') IS NOT NULL DROP TABLE DBO.loyalty_dev_summary_10y;
...
FROM VISIT_DIMENSION V
  ...
  LEFT JOIN VISITTYPE VT
    ON V.INOUT_CD = VT.C_BASECODE 
      AND CONVERT(DATE,V.START_DATE) >= dateadd(YY,-10, @indexDate) AND CONVERT(DATE,V.START_DATE) < @indexDate
...
(
  ...
  FROM (
    ...
    FROM (
      ...
      AND o.START_DATE >=  dateadd(YY,-10, @indexDate)
...
INTO DBO.loyalty_dev_summary_10y
FROM (
