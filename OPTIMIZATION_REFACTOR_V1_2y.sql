IF OBJECT_ID(N'DBO.usp_LoyaltyCohort_opt_2y') IS NOT NULL DROP PROCEDURE DBO.usp_LoyaltyCohort_opt_2y
GO

CREATE PROC DBO.usp_LoyaltyCohort_opt_2y
    @indexDate datetime
AS
...
IF OBJECT_ID(N'DBO.loyalty_dev_summary_2y', N'U') IS NOT NULL DROP TABLE DBO.loyalty_dev_summary_2y;
...
FROM VISIT_DIMENSION V
  ...
  LEFT JOIN VISITTYPE VT
    ON V.INOUT_CD = VT.C_BASECODE 
      AND CONVERT(DATE,V.START_DATE) >= dateadd(YY,-2, @indexDate) AND CONVERT(DATE,V.START_DATE) < @indexDate
...
(
  ...
  FROM (
    ...
    FROM (
      ...
      AND o.START_DATE >=  dateadd(yy,-2, @indexDate)
...
INTO DBO.loyalty_dev_summary_2y
FROM (
...
SELECT * FROM DBO.loyalty_dev_summary_2y WHERE Summary_Description = 'PercentOfSubjects' ORDER BY CUTOFF_FILTER_YN, TABLENAME;