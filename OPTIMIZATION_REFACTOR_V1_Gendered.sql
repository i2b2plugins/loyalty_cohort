--IF OBJECT_ID(N'DBO.usp_LoyaltyCohort_opt_gender') IS NOT NULL DROP PROCEDURE DBO.usp_LoyaltyCohort_opt_gender
--GO

--CREATE PROC DBO.usp_LoyaltyCohort_opt_gender
--    @indexDate datetime
--AS
...
IF OBJECT_ID(N'DBO.loyalty_dev_summary_gender', N'U') IS NOT NULL DROP TABLE DBO.loyalty_dev_summary_gender;
...
SELECT DISTINCT CONCEPT_CD --DISTINCT CASE WHEN CHARINDEX(':',CONCEPT_CD) > 0 THEN SUBSTRING(CONCEPT_CD,1,CHARINDEX(':',CONCEPT_CD)-1) ELSE CONCEPT_CD END AS CONCEPT_PREFIX
INTO #DEMCONCEPT
FROM CONCEPT_DIMENSION
WHERE CONCEPT_PATH LIKE '\ACT\Demographics%'
  AND CONCEPT_CD != ''

/* TESTING PERFORMANCE OF ETC-DEMOGRAPHIC LOOKUP */

--SELECT DISTINCT PATIENT_NUM
--FROM OBSERVATION_FACT O
--  LEFT JOIN #DEMCONCEPT C
--    ON O.CONCEPT_CD = C.CONCEPT_CD
--WHERE CONVERT(DATE,O.START_DATE) >= '20120101' 
--  AND C.CONCEPT_CD IS NULL
...
SELECT V.PATIENT_NUM
    , REPLACE(P.SEX_CD,'DEM|SEX:','') AS SEX
...
FROM VISIT_DIMENSION V
  ...
  JOIN (SELECT DISTINCT PATIENT_NUM
        FROM OBSERVATION_FACT O
          LEFT JOIN #DEMCONCEPT C
            ON O.CONCEPT_CD = C.CONCEPT_CD
        WHERE CONVERT(DATE,O.START_DATE) >= '20120101' 
          AND C.CONCEPT_CD IS NULL
        ) NONDEMFACT /* AT LEAST ONE NON-DEMOGRAPHIC FACT AFTER 2012 */
...
GROUP BY V.PATIENT_NUM, P.SEX_CD, FLOOR(DATEDIFF(DD,BIRTH_DATE,@indexDate)/365.25)
...
from ACT_ICD10CM_DX_V4 d, concept_dimension c
where d.C_FULLNAME = C.CONCEPT_PATH
  and (c_basecode is not null and c_basecode <> '')
UNION
Select distinct concept_cd 
from ACT_ICD9CM_DX_V4 d, concept_dimension c
where d.C_FULLNAME = C.CONCEPT_PATH
  and (c_basecode is not null and c_basecode <> '')
UNION
...
SELECT CUTOFF_FILTER_YN, Summary_Description, COHORTAGG.AGEGRP as tablename, TotalSubjects, TotalSubjects_Female, TotalSubjects_Male, Num_DX1, Num_DX2, MedUSe1, MedUse2, Mammography, PapTest, PSATest, Colonoscopy, FecalOccultTest
...
INTO DBO.loyalty_dev_summary_gender
FROM (
...
count(distinct IIF(SEX='F',patient_num,NULL)) AS TotalSubjects_Female,
count(distinct IIF(SEX='M',patient_num,NULL)) AS TotalSubjects_Male,
...
sum(cast(IIF(SEX='F',[Mammography],NULL) as int)) as Mammography,
sum(cast(IIF(SEX='F',[PapTest],NULL)     as int)) as PapTest,
sum(cast(IIF(SEX='M',[PSATest],NULL)     as int)) as PSATest,
...
100*avg(cast(IIF(SEX='F',[Mammography],NULL) as numeric(2,1))) as Mammography,
100*avg(cast(IIF(SEX='F',[PapTest],NULL)     as numeric(2,1))) as PapTest,
100*avg(cast(IIF(SEX='M',[PSATest],NULL)     as numeric(2,1))) as PSATest,
...
SELECT * FROM DBO.loyalty_dev_summary_gender WHERE Summary_Description = 'PercentOfSubjects' ORDER BY CUTOFF_FILTER_YN, TABLENAME;