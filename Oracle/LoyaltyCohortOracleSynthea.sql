...
COMMIT;
--DROP TABLE LOYALTY_COHORT_FINAL;
--SELECT * FROM LOYALTY_COHORT_FINAL; --999998 LOYALTY_TEST_DATA	31-MAR-22
select * from loyalty_dev;
declare
v_sql LONG;
begin
...
FROM DUAL;
COMMIT;
select * from constants; --31-MAR-22	1	2	31-MAR-12	UPITT	1
commit;
--set define off
...
--select * from LOYALTY_COHORT_AGEGRP where mdvisit_pname3 = 1;-- order by predicted_score desc;
--select * from LOYALTY_COHORT_AGEGRP where ROUTINE_CARE_2 = 1;-- order by predicted_score desc;
--select COUNT(*) from LOYALTY_COHORT_AGEGRP  WHERE SEX_CD = 'M' AND PREDICTED_SCORE > 0.4;--ORDER BY PREDICTED_SCORE DESC; --WHERE COHORT_NAME IS NOT NULL;-- ORDER BY PREDICTED_SCORE DESC; 999983 IN 1 YR LOOKBACK 170196

-- CREATE COHORT TABLE THAT REMOVES PEOPLE UNDER 18, 'EFFEMORAL' PATIENTS 
DROP TABLE MULTVISIT_PATIENTS;
CREATE TABLE MULTVISIT_PATIENTS AS SELECT * FROM (
...
FROM synthea_addon_vis_dim v inner join LOYALTY_COHORT c on c.patient_num=v.patient_num 
...
--SELECT * FROM LOYALTY_COHORT WHERE PATIENT_NUM NOT IN ( SELECT PATIENT_NUM FROM MULTVISIT_PATIENTS);

--SELECT * FROM synthea_addon_vis_dim WHERE PATIENT_NUM = 683302;
...
DROP TABLE MORE_THAN_DEMO_FACT_PTS;
CREATE TABLE MORE_THAN_DEMO_FACT_PTS AS SELECT * FROM (
...
SELECT COUNT(*) FROM MORE_THAN_DEMO_FACT_PTS;  --955734
SELECT COUNT(*) FROM MULTVISIT_PATIENTS;  --951095
SELECT COUNT(*) FROM MORE_THAN_DEMO_FACT_PTS;  --955734
/*
DELETE FROM LOYALTY_COHORT WHERE PATIENT_NUM NOT IN (
SELECT PATIENT_NUM FROM MORE_THAN_DEMO_FACT_PTS
INTERSECT 
SELECT PATIENT_NUM FROM MULTVISIT_PATIENTS
); --951095
*/
DROP TABLE LOYALTY_COHORT_FINAL;
...
INNER JOIN SYNTHEA_ADDON_PAT_DIM P ON P.PATIENT_NUM = F.PATIENT_NUM --951095
...
FROM COHORT_IN_PERIOD LC
JOIN synthea_addon_vis_dim V ON V.PATIENT_NUM = LC.PATIENT_NUM
JOIN VISIT_FEATURES F ON F.CONCEPT_CD = V.INOUT_CD
JOIN xref_LoyaltyCode_PSCoeff C ON C.FIELD_NAME = F.FEATURE_NAME
CROSS JOIN CONSTANTS x
WHERE CODE_TYPE = 'VISIT' AND 
--V.START_DATE between '01-JAN-16' and '01-JAN-21' --add_months( trunc(C.INDEXDATE), -12*X.LOOKBACKYEARS) and C.INDEXDATE
V.START_DATE between add_months( trunc(LC.INDEXDATE), -12*X.LOOKBACKYEARS) and LC.INDEXDATE --****michele fix
...
FROM COHORT_IN_PERIOD LC
CROSS JOIN CONSTANTS x
JOIN synthea_obs_fact O ON O.PATIENT_NUM = LC.PATIENT_NUM
JOIN FEATURES F ON F.CONCEPT_CD = O.CONCEPT_CD
JOIN xref_LoyaltyCode_PSCoeff C ON C.FIELD_NAME = F.FEATURE_NAME
...
COMMIT;
--select * from synthea_obs_fact where patient_num = 496587 order by start_date desc;
...
-- INSERT PATIENT COUNTS FILTERED ALL
INSERT INTO COHORT_AGG
...
-- INSERT PATIENT PERCENTS FILTERED ALL
INSERT INTO COHORT_AGG
SELECT
COHORT_NAME,
'Y' AS CUTOFF_FILTER_YN,
...
group by COHORT_NAME; --, CAG.AGEGRP
COMMIT;
--SELECT * FROM COHORT_AGG order by agegrp, cutoff_filter_yn, summary_description;
--delete from COHORT_AGG where agegrp = 'All Patients';

--UNFILTERED PATIENT COUNTS ALL
INSERT INTO COHORT_AGG
--NOT FILTERED BY PREDICTIVE CUTOFF 
SELECT
COHORT_NAME,
'N' AS CUTOFF_FILTER_YN,
'Patient Counts' as Summary_Description,
...
from loyalty_cohort_agegrp CAG 
CROSS JOIN CONSTANTS X
JOIN loyalty_AGEGRP_PSC P ON CAG.AGEGRP = P.AGEGRP
group by COHORT_NAME;--, CAG.AGEGRP
COMMIT;
--SELECT * FROM COHORT_AGG;

--unfiltereD percents all
INSERT INTO COHORT_AGG
SELECT
COHORT_NAME,
'N' AS CUTOFF_FILTER_YN,
...
COMMIT;
--TRUNCATE TABLE loyalty_dev_summary;
--select * from loyalty_cohort_agegrp where cohort_name = 'LOYALTY_TEST_DATA' and agegrp not in ('Over 65');
