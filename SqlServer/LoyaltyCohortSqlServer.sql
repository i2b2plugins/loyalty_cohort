...
To run, EXEC usp_LoyaltyCohort_opt @indexDate = '20210201', @SITE='STE', @LOOKBACK_YEARS=1,  @demographic_facts=0, @GENDERED=0, @filter_by_existing_cohort=0, @COHORT_FILTER=@cfilter, @output=0
...
It is ok under the SHRINE IRB to export this: select * FROM LOYALTY_DEV_SUMMARY where SUMMARY_DESCRIPTION='PERCENT SUBJECTS'
...
FROM #INCLPAT P
  ...
  JOIN DBO.VISIT_DIMENSION V 
    ...
    AND V.INOUT_CD IN (SELECT VD.C_BASECODE
                        FROM DBO.ACT_VISIT_DETAILS_V4 VD
                          JOIN DBO.XREF_LOYALTYCODE_PATHS LCP
                            ON VD.C_FULLNAME LIKE CONCAT(LCP.ACT_PATH,'%')
...
/* Calculate averAGE fact counts over Agegroups 
  THIS IS EXTREMELY SLOW ON LARGE DATA SO IS DISABLED
SET @STEP_TS = GETDATE()
...
INSERT INTO DBO.LOYALTY_DEV_SUMMARY (COHORT_NAME, SITE, LOOKBACK_YR, GENDER_DENOMINATORS_YN, CUTOFF_FILTER_YN, SUMMARY_DESCRIPTION, TABLE_NAME, NUM_DX1, NUM_DX2, MED_USE1, MED_USE2
...
SELECT DISTINCT COHORTAGG.COHORT_NAME, @SITE, @LOOKBACK_YEARS, IIF(@GENDERED=0,'N','Y') as GENDER_DENOMINATORS_YN, COHORTAGG.CUTOFF_FILTER_YN, SUMMARY_DESCRIPTION, COHORTAGG.AGE_GRP as TABLE_NAME, NUM_DX1, NUM_DX2, MED_USE1, MED_USE2
  ...
  , FC.AVG_FACT_COUNT as AVERAGE_FACT_COUNT
FROM (
