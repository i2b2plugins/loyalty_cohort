...
BEGIN
  ...
  WHERE F.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-@lookbackYears,CF.INDEX_DT)) AND F.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,CF.INDEX_DT)) /* ALTERED OCT2022 - BASE NON-DEMOGRAPHIC FACT LOOKUP ON MEASURE PERIOD */
  EXCEPT
...
FROM #INCLPAT P
  ...
  JOIN DBO.VISIT_DIMENSION V 
    ...
    AND V.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-@lookbackYears,CF.INDEX_DT)) AND V.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,CF.INDEX_DT)) /* ALTERED OCT2022 - BASE NON-DEMOGRAPHIC FACT LOOKUP ON MEASURE PERIOD */
    AND V.INOUT_CD IN (SELECT CD.CONCEPT_CD 
                        FROM DBO.CONCEPT_DIMENSION CD
...
from DBO.xref_LoyaltyCode_paths L, DBO.CONCEPT_DIMENSION c
...
WHERE O.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-@lookbackYears,C.INDEX_DT)) AND O.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,C.INDEX_DT)) /* ALTERED OCT2022 - BASE NON-DEMOGRAPHIC FACT LOOKUP ON MEASURE PERIOD */
...
FROM #cohort_agegrp cag
  ...
  join DBO.OBSERVATION_FACT O
    ON cag.patient_num = O.PATIENT_NUM
WHERE O.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-@lookbackYears,cag.INDEX_DT)) AND O.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,cag.INDEX_DT)) /* ALTERED OCT2022 - BASE NON-DEMOGRAPHIC FACT LOOKUP ON MEASURE PERIOD */
...
FROM DBO.xref_LoyaltyCode_CHARLSON C
  JOIN DBO.ACT_ICD10CM_DX_V4 DX10
