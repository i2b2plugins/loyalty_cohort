...
BEGIN
  ...
  WHERE START_DATE >= CONVERT(DATETIME,DATEADD(YY,-5,CF.INDEX_DT)) AND F.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,CF.INDEX_DT)) /* ALTERED OCT2022 - BASE NON-DEMOGRAPHIC FACT LOOKUP ON MEASURE PERIOD */
  EXCEPT
...
FROM #INCLPAT P
  ...
  JOIN VISIT_DIMENSION V 
    ...
    AND V.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-5,CF.INDEX_DT)) AND V.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,CF.INDEX_DT))
...
WHERE O.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-5,C.INDEX_DT)) AND O.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,C.INDEX_DT))
...
WHERE O.START_DATE >= CONVERT(DATETIME,DATEADD(YY,-5,cag.INDEX_DT)) AND O.START_DATE < CONVERT(DATETIME,DATEADD(DD,1,cag.INDEX_DT))
