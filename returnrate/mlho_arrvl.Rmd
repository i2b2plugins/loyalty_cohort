...
Issues: does not save ROC plots correctly. correlation results are output to screen and not file. testing needed. 
...
drv <- JDBC("com.microsoft.sqlserver.jdbc.SQLServerDriver",
            ...
            identifier.quote="`")
conn <- dbConnect(drv,
                  "jdbc:sqlserver://db.partners.org;databaseName=ACT","user","pass")
# ** THIS IS FOR ORACLE
...
cor.dat %>% group_by(decile) %>% dplyr::summarize(aver=mean(Predicted_score))
...
cor.test(cor.dat$Predicted_score,as.numeric(cor.dat$label_binary)) # Score to return (binary)
cor.test(cor.dat$decile,as.numeric(cor.dat$label_binary)) # Decile to return (binary)
...
cor.test(cor.dat$decile,cor.dat$CHARLSON_INDEX) # This is the one we're reporting in the paper
...
```{r Save}
results <- list(
  model.test = model.test, # MLHO model
  ...
  charlson.compare.summary2, # Charlson comparison plot data
  features.contrib # Importance in original vs. MLHO
